...
UPDATE users SET role = 'franqueado' WHERE lower(translate(role, 'áàãâéèêíìóòõôúùüçÁÀÃÂÉÈÊÍÌÓÒÕÔÚÙÜÇ', 'aaaaeeeiiooooouucAAAAEEEIIOOOOUUUC')) ILIKE '%franquea%';
-- Qualquer role que AINDA nao se encaixe vira funcionario
UPDATE users SET role = 'funcionario' WHERE role IS NULL OR role NOT IN ('funcionario', 'gerente', 'admin', 'caixa', 'franqueado');

-- Aplicar novo CHECK constraint
ALTER TABLE users ADD CONSTRAINT users_role_check
  CHECK (role IN ('funcionario', 'gerente', 'admin', 'caixa', 'franqueado'));
...
CREATE TABLE IF NOT EXISTS trails (
  ...
  target_roles TEXT[] DEFAULT ARRAY['funcionario','gerente','admin','caixa','franqueado'],
...
DO $$ BEGIN
  ALTER TABLE trails ADD COLUMN IF NOT EXISTS target_roles TEXT[] DEFAULT ARRAY['funcionario','gerente','admin','caixa','franqueado'];
EXCEPTION WHEN others THEN NULL;
END $$;
DO $$ BEGIN
...
END $$;
DO $$ BEGIN
  ALTER TABLE achievements ADD COLUMN IF NOT EXISTS icon VARCHAR(10) DEFAULT '🏆';
EXCEPTION WHEN others THEN NULL;
END $$;
DO $$ BEGIN
